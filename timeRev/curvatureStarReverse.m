function [ data, g, data0 ] = curvatureStarReverse(accuracy,revFlow,displayType)
% curvatureStarReverse: demonstrate motion by mean curvature on star interface.
%
%   [ data, g, data0 ] = curvatureStarReverse(accuracy, revFlow, displayType)
%
...
%                  'high'        Use odeCFL3.
%
%   revFlow: Boolean.  Use the time dependent version of the flow
%   field, which reverses the direction of motion at half-way time
%   to see if the advection equation is time-reversible.  Otherwise, the
...
% Make sure we can see the kernel m-files.
  addpath(genpath('../../LStoolbox/Kernel'));
...
if(useTimeRev)
  ...
  schemeData.one =  bValue * ones(size(theta));
  schemeData.two = -bValue * ones(size(theta));
else
